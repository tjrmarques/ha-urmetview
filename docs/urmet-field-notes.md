...
**Fix:** `async_ensure_stream()` now takes `mark_activity: bool = True`. `camera.py`'s snapshot path passes `mark_activity=False` — a snapshot can still start video fresh if it's not already running (needed either way, since the device has no separate lightweight "grab one frame" command), but repeated snapshot polling once video is already up no longer resets the idle clock on its own. The grace period is preserved for the case that actually needs it: video wasn't running yet, so `_mark_activity()` still fires even for a snapshot call, giving its own brief ffmpeg connection time to attach as a real consumer before the idle monitor's next 2s poll could otherwise tear it back down. `stream_source()` (genuine live view) is unaffected — still marks activity on every call, as it should.

**Status:** fixed, not yet re-validated live (found and fixed in the same session it was reported; the user was about to reload and re-test when this was written). No automated test yet, same gap as §15.
